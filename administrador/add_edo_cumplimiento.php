<?php
$page_title = 'Agregar Estado de Cumplimiento';
...
if (isset($_POST['add'])) {
    ...
    if (empty($errors)) {
        ...
        $query  = "INSERT INTO cat_edo_cumpl (";
        ...
        if ($db->query($query)) {
            //sucess
            $session->msg('s', "Estado de Cumplimiento creada! ");
			insertAccion($user['id_user'],'"'.$user['username'].'" creo el Estado de Cumplimiento -'.$name.'-.',1);
            redirect('add_edo_cumplimiento.php', false);
        } else {
            //failed
            $session->msg('d', 'Lamentablemente no se pudo dar de alta el Estado de Cumplimiento!');
            redirect('add_edo_cumplimiento.php', false);
        }
...
<div class="login-page">
    ...
    <form method="post" action="add_edo_cumplimiento.php" class="clearfix">
