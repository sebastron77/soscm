<?php
$page_title = 'Agregar Estado Procesal';
...
if (isset($_POST['add'])) {
    ...
    if (empty($errors)) {
        ...
        $query  = "INSERT INTO cat_est_procesal (";
        ...
        if ($db->query($query)) {
            //sucess
            $session->msg('s', "Estado Procesal creado! ");
			insertAccion($user['id_user'],'"'.$user['username'].'" creo el Estado Procesal -'.$name.'-.',1);
            redirect('add_edo_procesal.php', false);
        } else {
            //failed
            $session->msg('d', 'Lamentablemente no se pudo dar de alta el Estado Procesal!');
            redirect('add_edo_procesal.php', false);
        }
...
<div class="login-page">
    <div class="text-center">
        <h3>Agregar Estado Procesal</h3>
    </div>
    <?php echo display_msg($msg); ?>
    <form method="post" action="add_edo_procesal.php" class="clearfix">
