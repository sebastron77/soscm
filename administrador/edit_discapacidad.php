<?php
$page_title = 'Editar Discapacidad';
require_once('includes/load.php');
$user = current_user();
page_require_level(1);
?>
<?php
$e_comunidad = find_by_id('cat_discapacidades', (int)$_GET['id'], 'id_cat_disc');
if (!$e_comunidad) {
...
if (isset($_POST['update'])) {
    ...
    if (empty($errors)) {
        ...
        $query  = "UPDATE cat_discapacidades SET ";
        $query .= "descripcion='{$name}' ";
        $query .= "WHERE id_cat_disc='{$db->escape($e_comunidad['id_cat_disc'])}'";
		 
		$result = $db->query($query);
        if ($result && $db->affected_rows() === 1) {
            //sucess
            $session->msg('s', "Discapacidad ha actualizada! '".($name)."'");
			insertAccion($user['id_user'],'"'.$user['username'].'" edito la discapacidad '.$name.'(id:'.(int)$e_comunidad['id_cat_disc'].').',2);
            redirect('edit_discapacidad.php?id=' . (int)$e_comunidad['id_cat_disc'], false);
        } else {
            //failed
...
<div class="login-page">
    ...
    <form method="post" action="edit_discapacidad.php?id=<?php echo (int)$e_comunidad['id_cat_disc']; ?>" class="clearfix">
