...
$denuncia = find_by_id('recursos_decuncias', (int)$_GET['id'], 'id_recursos_decuncias');
...
if (isset($_POST['update'])) {
	if (empty($errors)) {
		$id = (int)$denuncia['id_recursos_decuncias'];			
        ...
        $sentido_resolucion   = remove_junk($db->escape($_POST['sentido_resolucion']));
			...
			$result = $db->query($sql);
				...
				insertAccion($user['id_user'], '"' . $user['username'] . '" dió resolución a la Denuncia('.$id.') de Folio: -' . $denuncia['folio_accion'].'-', 2);
				$session->msg('s', "La Denuncia con folio '" . $denuncia['folio_accion'] . "' se le ha dado Resolución con éxito.");
...
<div class="login-page2">
    ...
    <form method="post" action="resolucion_recurso.php?id=<?php echo (int)$denuncia['id_recursos_decuncias']; ?>"  class="clearfix" enctype="multipart/form-data">
        <div class="form-group">
            <label for="area-name" class="control-label">Folio Recurso</label>
            <input type="name" class="form-control" name="area-name" value="<?php echo ucwords($denuncia['folio_accion']); ?>" readonly >
        </div>
        <div class="form-group">
