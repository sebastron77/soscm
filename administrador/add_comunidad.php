<?php
$page_title = 'Agregar Comunidad Indigena';
...
if (isset($_POST['add'])) {
    ...
    if (empty($errors)) {
        ...
        $query  = "INSERT INTO cat_comunidades (";
        ...
        if ($db->query($query)) {
			...
			insertAccion($user['id_user'],'"'.$user['username'].'" creo la comunidad indígena -'.$name.'-.',1);
            redirect('add_comunidad.php', false);
        } else {
            //failed
            $session->msg('d', 'Lamentablemente no se pudo dar de alta la comunidad indiogena!');
            redirect('add_comunidad.php', false);
        }
...
<div class="login-page">
    <div class="text-center">
        <h3>Agregar Comunidad Indígena</h3>
    </div>
    <?php echo display_msg($msg); ?>
    <form method="post" action="add_comunidad.php" class="clearfix">
