                ...
                <table class="datatable table table-bordered table-striped">
                    ...
                    <tbody>
                        <?php foreach ($quejas_libro as $queja) : ?>
                            <tr>
                                <td><?php echo remove_junk(ucwords($queja['folio_queja'])) ?></td>
                                <?php
                                $folio_editar = $queja['folio_queja'];
                                $resultado = str_replace("/", "-", $folio_editar);
                                ?>
                                <td><?php echo remove_junk(ucwords($queja['fecha_presentacion'])) ?></td>
                                <td><?php echo remove_junk(ucwords($queja['medio_pres'])) ?></td>
                                <td><?php echo remove_junk(ucwords($queja['nombre_autoridad'])) ?></td>
                                <td><a target="_blank" style="color: #0094FF;" href="uploads/quejas/<?php echo $resultado . '/' . $queja['archivo']; ?>"><?php echo $queja['archivo']; ?></a></td>
                                <td><?php echo remove_junk(ucwords($queja['nombre_quejoso']." ".$queja['paterno_quejoso']." ".$queja['materno_quejoso'])) ?></td>                                
                                <td><?php echo remove_junk(ucwords($queja['estatus_queja'])) ?></td>
                                <?php if (($nivel <= 2) || ($nivel == 5)) : ?>
                                    <td class="text-center">
                                        <div class="btn-group">
                                            ...
                                            <?php if ($nivel == 1) : ?>
                                                <!-- <a href="delete_orientacion.php?id=<?php echo (int)$queja['id']; ?>" class="btn btn-delete btn-md" title="Eliminar" data-toggle="tooltip" onclick="return confirm('¿Seguro(a) que deseas eliminar esta orientación?');">
