...
if (isset($_POST['add_env_correspondencia'])) {
    ...
    if (empty($errors)) {
        ...
        if(($move && $name == '') && ($fecha_en_que_se_turna == '') && ($fecha_espera_respuesta == '')) {
            $query = "INSERT INTO envio_correspondencia (";
            $query .= "folio,fecha_emision,asunto,medio_envio,se_turna_a_area,tipo_tramite,observaciones,area_creacion";
            $query .= ") VALUES (";
            $query .= " '{$folio}','{$fecha_emision}','{$asunto}','{$medio_envio}','{$se_turna_a_area}','{$tipo_tramite}','{$observaciones}','{$area}'";
            $query .= ")";
