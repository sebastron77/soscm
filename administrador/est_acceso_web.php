...
$page_title = 'Estadística de Acceso a Módulos';
require_once('includes/load.php');

$user = current_user();
$nivel_user = $user['user_level'];
$accesos = web_accesos((int)$_GET['id']);
...
<div class="panel-body">
  ...
  <div id="prueba">
    <center>     
          <h3 style="margin-top: 10px; color: #3a3d44;">Estadística de Acceso a Módulos</h2>        
          <div class="row" style="display: flex; justify-content: center; align-items: center;">            
            <div style="width:40%; float:left;">
              ...
              <script>
                var yValues = [<?php foreach ($accesos as $gen) : ?><?php echo $gen['total']; ?>, <?php endforeach; ?>];
                ...
                const myChart = new Chart(ctx, {
                  type: 'bar',
                  data: {
                    labels: [<?php foreach ($accesos as $gen) : ?> '<?php echo $gen['modulo']; ?>', <?php endforeach; ?>],
                    datasets: [{
                      label: 'Accesos Módulos',
                      data: yValues,
                      backgroundColor: [
              ...
              <table class="table table-bordered table-striped">
                <thead class="thead-purple">
                  <tr style="height: 10px;">
                    <th class="text-center" style="width: 70%;">Módulo</th>
                    <th class="text-center" style="width: 30%;">Cantidad</th>
                  </tr>
                </thead>
                <tbody style="background: white;">
                  <?php $total = 0;
                  foreach ($accesos as $gen) : ?>
                    <tr>
                      <td>
                        <?php echo remove_junk(ucwords($gen['modulo'])) ?>
                      </td>
                      <td>
