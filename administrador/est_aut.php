...
$page_title = 'Estadísticas Autoridades';
require_once('includes/load.php');
$autoridades = autoridad((int)$_GET['id']);
...
<div class="panel-body">
    <center>
        ...
        <div id="prueba">
            <center>
                <?php if((int)$_GET['id'] == 1): ?>
                <h2 style="margin-top: 15px; color: #3a3d44;">Estadística de Orientaciones (Por Autoridad)</h2>
                <?php endif; ?>
                <?php if((int)$_GET['id'] == 2): ?>
                <h2 style="margin-top: 15px; color: #3a3d44;">Estadística de Canalizaciones (Por Autoridad)</h2>
                <?php endif; ?>
            </center>
            <div class=" row" style="display: flex; justify-content: center; align-items: center; margin-left:-70px;">
                ...
                <div style="width:50%; float:left;">
                    ...
                    <script>
                        var yValues = [<?php foreach ($autoridades as $autoridad) : ?><?php echo $autoridad['total']; ?>, <?php endforeach; ?>];
                        ...
                        const gVulnerableB = new Chart(ctx3, {
                            type: 'bar',
                            data: {
                                labels: [<?php foreach ($autoridades as $autoridad) : ?> '<?php echo $autoridad['descripcion']; ?>', <?php endforeach; ?>],
                                datasets: [{
                                    label: 'Orientaciones por Autoridad',
                                    data: yValues,
                                    backgroundColor: [
                    ...
                    <table class="table table-bordered table-striped">
                        <thead class="thead-purple">
                            <tr style="height: 10px;">
                                <th class="text-center" style="width: 70%;">Autoridades</th>
                                <th class="text-center" style="width: 30%;">Cantidad</th>
                            </tr>
                        </thead>
                        <tbody style="background: white;">
                            <?php $total=0;  foreach ($autoridades as $autoridad) : ?>
                                <tr>
                                    <td>
                                        <?php echo remove_junk(ucwords($autoridad['descripcion'])) ?>
                                    </td>
                                    <td>
