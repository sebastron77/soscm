...
if (isset($_POST['update'])) {
    if (empty($errors)) {
        ...
        if ($name != '') {
            ...
            id_area_solicitante='{$area_solicitante}',id_cat_der_vuln='{$derechos_vuln}',fecha_intervencion='{$fecha_intervencion}',resultado='{$resultado2}', documento_emitido='{$documento_emitido}', ficha_adjunto='{$name}', protocolo_estambul='{$protocolo_estambul}',nombre_especialista='{$nombre_especialista}', clave_documento='{$clave_documento}' WHERE id_ficha='{$db->escape($id)}'";
        }
        if ($name == '') {
            $sql = "UPDATE fichas SET id_cat_funcion='{$funcion}',id_visitaduria='{$visitaduria}',
            id_area_solicitante='{$area_solicitante}',id_cat_der_vuln='{$derechos_vuln}',fecha_intervencion='{$fecha_intervencion}',resultado='{$resultado2}',documento_emitido='{$documento_emitido}',protocolo_estambul='{$protocolo_estambul}',nombre_especialista='{$nombre_especialista}',clave_documento='{$clave_documento}' WHERE id_ficha='{$db->escape($id)}'";
        }
        $result = $db->query($sql);
