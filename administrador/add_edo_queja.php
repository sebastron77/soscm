<?php
$page_title = 'Agregar Estado de la Queja';
...
if (isset($_POST['add'])) {
    ...
    if (empty($errors)) {
        ...
        $query  = "INSERT INTO cat_estatus_queja (";
        ...
        if ($db->query($query)) {
            //sucess
            $session->msg('s', "Estado de la Queja creado! ");
			insertAccion($user['id_user'],'"'.$user['username'].'" creo el Estado de la Queja -'.$name.'-.',1);
            redirect('add_edo_queja.php', false);
        } else {
            //failed
            $session->msg('d', 'Lamentablemente no se pudo dar de alta el Estado de la Queja!');
            redirect('add_edo_queja.php', false);
        }
...
<div class="login-page">
    <div class="text-center">
        <h3>Agregar Estado de la Queja</h3>
    </div>
    <?php echo display_msg($msg); ?>
    <form method="post" action="add_edo_queja.php" class="clearfix">
